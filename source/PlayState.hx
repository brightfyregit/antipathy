...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			if (storyWeek == 0)
				detailsText = "Story Mode: ANTIPATHY | ";
			else
		...
		switch (SONG.song.toLowerCase())
		{
				  ...
				  case 'battered' | 'overdrive':
		          {
						defaultCamZoom = 0.7;
		                curStage = 'nevada';
						var sky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbgsky','shared'));
		                ...
		                var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbglandscape2','shared'));
		                bg.antialiasing = true;
		                ...
		                var bldg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbglandscape','shared'));
		                bldg.antialiasing = true;
		                bldg.scrollFactor.set(0.9, 0.9);
						bldg.scale.set(1.7, 1.7);
		                bldg.active = false;
		                add(bldg);
						
						var cliff:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbgcliffs','shared'));
		                cliff.antialiasing = true;
		                cliff.scrollFactor.set(0.9, 0.9);
						cliff.scale.set(1.7, 1.7);
		                cliff.active = false;
		                add(cliff);
						
						var floor:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbgfloor','shared'));
		...
		if (curStage == 'nevada')
		{
			var fog:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('antipathy/hankbgfog','shared'));
		    fog.antialiasing = true;
		    fog.scrollFactor.set(0.9, 0.9);
			fog.scale.set(1.7, 1.7);
		    fog.active = false;
		    add(fog);
		}
		...
		sicksTxt.text = "Mad: " + sicks;
		goodsTxt.text = "Nice: " + goods;
		badsTxt.text = "Sad: " + bads;
		shitsTxt.text = "Fucked: " + shits;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			introAssets.set('nevada', ['antipathy/count/ready', 'antipathy/count/get', 'antipathy/count/set','antipathy/count/go']);
...

			{
				case 0:
					if(curStage == 'nevada'){
					yo = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
					yo.scale.set(0.3, 0.3);
					yo.cameras=[camHUD];
					yo.scrollFactor.set();
					yo.updateHitbox();
					...
					yo.screenCenter();
					add(yo);
					FlxTween.tween(yo, {alpha: 0}, Conductor.crochet / 1000, {
				...
				case 1:
					if(curStage == 'nevada'){
					ready = new FlxSprite().loadGraphic(Paths.image(introAlts[1]));
					ready.scale.set(0.3, 0.3);
					}
					else{
				...
				case 2:
					if(curStage == 'nevada'){
					set = new FlxSprite().loadGraphic(Paths.image(introAlts[2]));
					set.scale.set(0.3, 0.3);}
					else{
				...
				case 3:
					if(curStage == 'nevada'){
					go = new FlxSprite().loadGraphic(Paths.image(introAlts[3]));
					go.scale.set(0.3, 0.3);}
					else{
		...
		if (curSong == 'Battered')
			{
				switch(curStep)
				{
					...
					var three:FlxSprite = new FlxSprite().loadGraphic(Paths.image('antipathy/count/three', 'shared'));
					three.scrollFactor.set();
					three.scale.set(0.5, 0.5);
					three.updateHitbox();
					three.screenCenter();
					three.y -= 100;
					three.alpha = 0.9;
					...
					case 1060:
					FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.3}, 0.7, {ease: FlxEase.cubeInOut});
					var two:FlxSprite = new FlxSprite().loadGraphic(Paths.image('antipathy/count/two', 'shared'));
					...
					case 1063:
					FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.3}, 0.7, {ease: FlxEase.cubeInOut});
					var one:FlxSprite = new FlxSprite().loadGraphic(Paths.image('antipathy/count/one', 'shared'));
					...
					var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image('antipathy/count/go-alt', 'shared'));
					go.scrollFactor.set();
					go.scale.set(0.5, 0.5);
					go.updateHitbox();
					go.screenCenter();
					go.y -= 100;
					go.alpha = 0.9;
